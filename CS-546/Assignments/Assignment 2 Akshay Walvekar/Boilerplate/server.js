...
app.get("/api/todo/open", function (request,response){
	var TaskStatus = "open";
	var TaskResult = myData.TaskBasedOnStatus(TaskStatus);
...
app.put("/api/todo/:id", function(request,response){
	...
	try{
	var taskResult = myData.UpdateTask(request.params.id,taskTitle,taskDescription,taskStatus);
	response.json(taskResult);
	}catch(e){
		response.status(e.code).json(e.error);
	}
...
app.delete("/api/todo/:id", function(request,response){
	...
	try{
		var taskResult = myData.DeleteTask(request.params.id);
